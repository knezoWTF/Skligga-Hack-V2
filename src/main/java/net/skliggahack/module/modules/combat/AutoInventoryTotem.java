...
public class AutoInventoryTotem extends Module implements PlayerTickListener
{
	...
	public void onPlayerTick()
	{
		...
		if (autoSwitch.get())
			inv.setSelectedSlot(totemSlot.get());
		if (activateOnKey.get() && !activateKeybind.isDown())
			return;
		if (inv.getStack(40).getItem() != Items.TOTEM_OF_UNDYING)
		{
		...
		ItemStack mainHand = inv.getStack(inv.getSelectedSlot());
		if (mainHand.isEmpty() ||
				forceTotem.get() && mainHand.getItem() != Items.TOTEM_OF_UNDYING)
		{
			...
			if (slot != -1)
			{
				mc.interactionManager.clickSlot(((InventoryScreen) mc.currentScreen).getScreenHandler().syncId, slot, inv.getSelectedSlot(), SlotActionType.SWAP, mc.player);
			}
		}
	}
