...
public class AutoLootYeeter extends Module implements PlayerTickListener
{
		...
		for (Entity e : collidedEntities)
		{
			if (e instanceof ItemEntity itemStack)
			{
				...
				if (item == Items.END_CRYSTAL ||
					...
					return true;
				if (isDiamondOrNetheriteGear(item))
					return true;
				}
			}
		}
	...
	private boolean isDiamondOrNetheriteGear(Item item)
	{
		return item == Items.DIAMOND_SWORD || item == Items.DIAMOND_PICKAXE
			...
			|| item == Items.DIAMOND_BOOTS || item == Items.NETHERITE_SWORD
	...
	private int findPearlSlot()
	{
		...
		for (int i = 9; i < 36; i++)
		{
			...
			if (itemStack.getItem() == Items.ENDER_PEARL)
			{
				if (fewestPearlSlot == -1 ||
						itemStack.getCount() < inv.getStack(fewestPearlSlot).getCount())
				{
					fewestPearlSlot = i;
				}
			}
		...
		if (fewestPearlSlot == -1)
			return -1;
		if (pearlCount - inv.getStack(fewestPearlSlot).getCount() >= minPearls.get())
		{
