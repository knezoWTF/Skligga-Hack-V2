...
public class AutoDoubleHand extends Module implements PlayerTickListener
{
	...
	private List<EndCrystalEntity> getNearByCrystals()
	{
		...
		return MC.world.getEntitiesByType(TypeFilter.instanceOf(EndCrystalEntity.class), new Box(pos.add(-6, -6, -6), pos.add(6, 6, 6)), a -> true);
	}
	...
	public void onPlayerTick()
	{
		...
		for (int i = 1; i <= f; i++)
			if (BlockUtils.hasBlock(MC.player.getBlockPos().add(0, -i, 0)))
				return;
		if (BlockUtils.hasBlock(BlockPos.ofFloored(new Vec3d(MC.player.getX(), MC.player.getY() - activatesAboveV, MC.player.getZ()))))
			return;
